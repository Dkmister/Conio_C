...
void movePlataformaUpDown(PLATAFORMA *p2,PLATAFORMA *p3){
    p3->vel *=-1;
    if(p3->vel == 1){
        apagaPlataforma((*p2));
        apagaPlataforma((*p3));
        if(p2->pos.y + p2->dir.y == 23){
            p2->dir.y -= p2->dir.y;
        }
        ...
        if (p2->pos.y <= 21)desenhaPlataforma((*p2));
        if (p3->pos.y >= 3)desenhaPlataforma((*p3));
    }
}

void moveJogador (JOGADOR *j, PLATAFORMA *p, PLATAFORMA *p1, PLATAFORMA *p2, PLATAFORMA *p3){
    int code = 3;
    apagaJogador((*j));
    if ((j->pos.x >= p->pos.x && j->pos.x <= p->pos.x + TAM && j->pos.y == p->pos.y -1) ||
        (j->pos.x >= p1->pos.x && j->pos.x <= p1->pos.x + TAM && j->pos.y == p1->pos.y -1)||
        (j->pos.x >= p2->pos.x && j->pos.x <= p2->pos.x + TAM && j->pos.y == p2->pos.y -1)||
        ...
        code = 2;
            ...
            if (j->pos.x >= p2->pos.x && j->pos.x <= p2->pos.x + TAM && j->pos.y == p2->pos.y -1){
                    switch (code){
                    case 0: j->pos.y += j->dir.y;desenhaJogador((*j));
                            break;
                    case 1: j->pos.x += j->dir.x +1; j->pos.y -= j->dir.y+3; if (j->pos.x != 9){desenhaJogador((*j));};
                            break;
                    case 2: j->pos.x -= j->dir.x; j->pos.y -= j->dir.y+3; if (j->pos.x != 9){desenhaJogador((*j));};
                            break;
                    case 3: j->pos.y -= j->dir.y+3; if (j->pos.x != 9){desenhaJogador((*j));};
                            break;
                }
            }
        }
...
int main(){
    do{
        ...
        Sleep(50);
        desenhaPlataforma((p2));
        Sleep(50);
        desenhaPlataforma((p3));
        Sleep(50);
        desenhaJogador((j));
        while(p.pos.x > 2 && p1.pos.x < 63){

            movePlataformaLR(&p, &p1);
            movePlataformaUpDown(&p2, &p3);
            Sleep(100);
            moveJogador((&j), (&p), (&p1),(&p2), (&p3));
        }
    }
